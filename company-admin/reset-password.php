...
$reset = $token ? pw_validate_token($token, 'company_admin') : null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $reset) {
    ...
    if (strlen($pw) < 8) {
        ...
        pw_consume_token((int) $reset['id'], 'company_admin', (int) $reset['user_id'], $pw);
        $done = true;
    }
}
?><!doctype html>
...
button { width:100%; padding: 13px; border:0; cursor:pointer; background:#10b981; color:#fff; font-weight:600; border-radius:10px; margin-top: 18px; font-size: 15px; }
...
<div class="card">
  ...
  <?php if ($done): ?>
    ...
    <a href="/company-admin/login.php"><button type="button">Sign in →</button></a>
  <?php elseif (!$reset): ?>
    <h2>Link not valid</h2>
    <p class="lead">This reset link is invalid, expired, or already used.</p>
    <a href="/company-admin/forgot-password.php"><button type="button">Request a new link</button></a>
  <?php else: ?>
